module.exports = scoreCalculator = (receipt) => {
    try {
        ...
        for(let i = 0; i < receipt.retailer.length; i++) {
            if( ((receipt.retailer[i] >= 'a')&&(receipt.retailer[i] <= 'z')) || ((receipt.retailer[i] >= 'A')&&(receipt.retailer[i] <= 'Z')) || ((receipt.retailer[i] >= '0')&&(receipt.retailer[i] <= '9')) ) {
                alphaNumbericCount++;
            }
        }
        let roundTotal = 0;
        ...
        return alphaNumbericCount + roundTotal + multipleTwentyFive + itemCountPoints + itemPoints + datePoints + timePoints
    } catch (error) {
        return error;
    }
}
