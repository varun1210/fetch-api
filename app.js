...
app.post('/receipts/process', (req, res, next) => {
    try {
        if(Object.keys(req.body).length == 0) {
            throw new errors.BadRequestError("INVALID RECEIPT: Received invalid format OR empty receipt")
        }
        ...
        if(receiptPoints instanceof Error) {
            throw new errors.BadRequestError("INVALID RECEIPT: One or more of the fields in this receipt is inconsistent")
        }
...
app.get('/receipts/:id/points', (req, res, next) => {
    try {
        ...
        if(!receipts.has(receiptID)) {
            throw new errors.BadRequestError("INVALID RECEIPT ID: Receipt ID does not exist")
        } else {
